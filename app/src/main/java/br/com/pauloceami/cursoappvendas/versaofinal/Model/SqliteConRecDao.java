...
public class SqliteConRecDao {
    ...
    public boolean gravar_parcela(SqliteConRecBean rec) {
        try {
            ...
            stmt.bindDouble(6, rec.getRec_valor_receber());
            stmt.bindDouble(7, rec.getRec_valorpago());
...
//***********************************************************************************
    ...
    public SqliteConRecBean busca_menor_parcela(String chave_da_venda) {
        ...
        try {
            sql = "select min(rec_numparcela) as rec_numparcela ,rec_cli_codigo,rec_cli_nome,vendac_chave,rec_datamovimento,rec_valor_receber,rec_valorpago,rec_datavencimento,rec_data_que_pagou,rec_recebeu_com,rec_enviado from CONREC where vendac_chave = ? and rec_valorpago = 0";
            cursor = db.rawQuery(sql, new String[]{chave_da_venda});
            if (cursor.moveToFirst()) {
                ...
                receber.setRec_valor_receber(cursor.getDouble(cursor.getColumnIndex(receber.VALOR_A_RECEBER)));
            }
        } catch (SQLiteException e) {
    ...
    public void baixar_parcela_cliente_integral(SqliteConRecBean rec) {
        try {
            ...
            stmt = db.compileStatement(sql);
            stmt.bindDouble(1, rec.getRec_valorpago());
    ...
    public void atualiza_valorparcela(SqliteConRecBean rec) {
        try {
            ...
            stmt = db.compileStatement(sql);
            stmt.bindDouble(1, rec.getRec_valor_receber());
            stmt.bindString(2, "N");
    ...
    public List<SqliteConRecBean> busca_parcelas_geradas_na_compra(String vendac_chave) {
        ...
        try {
            ...
            sql = "select * from CONREC where vendac_chave = ? order by rec_numparcela asc";
            cursor = db.rawQuery(sql, new String[]{vendac_chave});
            while (cursor.moveToNext()) {
                ...
                parcela.setRec_valor_receber(cursor.getDouble(cursor.getColumnIndex(parcela.VALOR_A_RECEBER)));
                parcela.setRec_valorpago(cursor.getDouble(cursor.getColumnIndex(parcela.VALOR_PAGO)));
    ...
    public List<SqliteConRecBean> busca_parcelas_do_cliente(Integer cli_codigo, String vendac_chave) {
        ...
        try {
            sql = "select * from CONREC where rec_cli_codigo = ? and  vendac_chave = ? and rec_valorpago = 0";
            cursor = db.rawQuery(sql, new String[]{cli_codigo.toString(), vendac_chave});
            while (cursor.moveToNext()) {
                SqliteConRecBean parcela = new SqliteConRecBean();
